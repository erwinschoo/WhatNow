// screens-quiz.jsx — Quiz hub, quiz flow (tap + swipe variants), result badge
const { useState: useStateQ, useRef: useRefQ, useEffect: useEffectQ } = React;

// ── Quiz hub (the Quiz tab) ──────────────────────────────────────────
function QuizHub() {
  ...
  return (
    <div className="wn-scroll" style={{ position: 'absolute', inset: 0, padding: '60px 18px 100px' }}>
      ...
      <button onClick={() => startQuiz('all')} style={{ display: 'block', width: '100%', textAlign: 'left', cursor: 'pointer', border: '1px solid var(--line2)', borderRadius: 22, padding: 22, position: 'relative', overflow: 'hidden', background: 'linear-gradient(150deg, #2a1606, #120a04)' }}>
        ...
        <div style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: 24, color: '#fff', marginTop: 8, lineHeight: 1.05, textWrap: 'balance' }}>Hoe goed ken jij<br />de cinema?</div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, var(--bg2), var(--bg))', zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'wnFade .25s' }}>
      ...
      <div className="wn-scroll" style={{ flex: 1, padding: '28px 20px 16px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: 25, lineHeight: 1.15, color: 'var(--tx)', marginTop: 10, textWrap: 'balance' }}>{q.q}</div>

        <div style={{ flex: 1 }} />

        {t.quizStyle === 'swipe'
          ? <SwipeOptions key={i} q={q} picked={picked} onPick={choose} motion={!t.reduceMotion} />
        ...
        {picked != null && (
          <div style={{ marginTop: 18, animation: 'wnFadeUp .3s' }}>
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: 14, display: 'flex', gap: 11 }}>
              <div style={{ width: 38, height: 56, flexShrink: 0, borderRadius: 8 }}><Poster film={FILM_BY_ID[q.film]} showText={false} rounded={8} /></div>
              <div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 9.5, letterSpacing: 1, color: 'var(--amber2)' }}>WIST JE DAT?</div>
                <div style={{ fontFamily: 'var(--sans)', fontSize: 13, lineHeight: 1.4, color: 'var(--tx2)', marginTop: 5, textWrap: 'pretty' }}>{q.fact}</div>
              </div>
            </div>
...
// tap-grid options
function TapOptions({ q, picked, correct, onPick }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 11 }}>
      ...
      <style>{`@keyframes wnShake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}75%{transform:translateX(6px)}}`}</style>
    </div>
  );
}

// swipe-card options — swipe right to lock answer, left to pass
function SwipeOptions({ q, picked, onPick, motion }) {
  const [top, setTop] = useStateQ(0); // index in option order currently on top
  const order = useRefQ(q.options.map((_, i) => i));
  const [dx, setDx] = useStateQ(0);
  const start = useRefQ(null);
  if (picked != null) {
    // collapsed: just show locked summary
    return (
  ...
  const up = () => {
    if (dx > 80) onPick(cur);
    else if (dx < -80) setTop(t => (t + 1) % order.current.length);
...
// ── Result ───────────────────────────────────────────────────────────
function QuizResult({ score, total, onClose, questions }) {
  ...
  useEffectQ(() => { saveQuizScore(pct); }, []);
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(700px 500px at 50% 8%, rgba(255,138,43,0.18), transparent 60%), var(--bg)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', animation: 'wnFade .3s', padding: '70px 22px 24px', overflowY: 'auto' }} className="wn-scroll">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          {questions.map((q, i) => (
            <div key={i} style={{ display: 'flex', gap: 11, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 11 }}>
              <div style={{ width: 32, height: 48, flexShrink: 0, borderRadius: 7 }}><Poster film={FILM_BY_ID[q.film]} showText={false} rounded={7} /></div>
              <div style={{ fontFamily: 'var(--sans)', fontSize: 12, lineHeight: 1.38, color: 'var(--tx2)', textWrap: 'pretty' }}>{q.fact}</div>
            </div>
          ))}
        </div>
      </div>
...
Object.assign(window, { QuizHub, QuizFlow });
