// components.jsx — shared context, icons, and UI primitives for WhatNow
const { useState, useEffect, useRef, useContext, createContext } = React;

// Shared app context (provided by app.jsx, consumed everywhere)
const WN = createContext(null);
const useWN = () => useContext(WN);

// ── Icons (simple line glyphs, 24px grid, currentColor) ──────────────
function Icon({ name, size = 24, stroke = 1.8, fill = 'none', style }) {
...
// ── Poster (styled placeholder standing in for real artwork) ─────────
function Poster({ film, style, rounded = 14, showText = true, badge }) {
  const [a, b] = film.grad;
  return (
    <div style={{
      ...
      {/* faux film-grain stripes */}
      <div style={{
        position: 'absolute', inset: 0, opacity: 0.5, mixBlendMode: 'soft-light',
        backgroundImage: 'repeating-linear-gradient(115deg, rgba(255,255,255,0.07) 0 1px, transparent 1px 5px)',
      }} />
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(120% 80% at 50% 0%, rgba(255,255,255,0.10), transparent 55%)` }} />
      {/* motif ring */}
      <div style={{
        position: 'absolute', right: -28, top: -28, width: 120, height: 120, borderRadius: '50%',
        border: `1.5px solid ${film.ink}`, opacity: 0.28,
      }} />
      {showText && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '14px 13px',
          ...
          <div style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: 15.5, lineHeight: 1.06, color: '#fff', marginTop: 3, textWrap: 'balance' }}>{film.title}</div>
        </div>
      )}
      <div style={{ position: 'absolute', top: 9, left: 10, fontFamily: 'var(--mono)', fontSize: 8.5, letterSpacing: 1.5, color: 'rgba(255,255,255,0.55)' }}>WHATNOW</div>
      {badge}
    </div>
  );
}
...
function ScorePill({ src, value, compact }) {
  const meta = {
    imdb: { tag: 'IMDb', txt: '#f5c518', fmt: v => v.toFixed(1) },
    rt: { tag: 'RT', txt: '#fa6f4d', fmt: v => v + '%' },
    mc: { tag: 'MC', txt: '#9fd356', fmt: v => String(v) },
  }[src];
  return (
    <div style={{
...
// ── Theme chip ───────────────────────────────────────────────────────
function ThemeChip({ label, active, onClick, size = 'm' }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      fontFamily: 'var(--mono)', fontSize: fs, fontWeight: 500, letterSpacing: 0.2,
...
// ── Facet chip (filter toggle with optional count) ───────────────────
...
// ── Glow CTA button ──────────────────────────────────────────────────
function GlowButton({ children, onClick, variant = 'amber', size = 'm', icon, style, full }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      padding: s.p, borderRadius: 999, cursor: 'pointer', width: full ? '100%' : undefined,
      fontFamily: 'var(--sans)', fontSize: s.fs, fontWeight: 600, letterSpacing: 0.1,
      transition: 'transform .12s ease, box-shadow .2s ease', WebkitTapHighlightColor: 'transparent',
...
function FeelSlider({ feel, value, onChange }) {
  return (
    <div style={{ padding: '2px 0' }}>
      ...
      <div style={{ position: 'relative', height: 24, display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{
          position: 'absolute', left: `calc(${value * 10}% - 11px)`, width: 22, height: 22, borderRadius: '50%',
          background: '#fff', border: '3px solid var(--amber)', boxShadow: '0 0 14px rgba(255,138,43,0.55)', pointerEvents: 'none',
        }} />
      </div>
...
function StatRing({ pct, size = 116, stroke = 9, label, value, sub }) {
    ...
    <div style={{ position: 'relative', width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="url(#ringGrad)" strokeWidth={stroke}
          strokeLinecap="round" strokeDasharray={c} strokeDashoffset={off} style={{ transition: 'stroke-dashoffset 1s cubic-bezier(.2,.8,.2,1)' }} />
        <defs>
          <linearGradient id="ringGrad" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#FF7A18" /><stop offset="1" stopColor="#FFB347" />
          </linearGradient>
        </defs>
      </svg>
...
// ── Feel-match calc (distance → 0..100) ──────────────────────────────
function feelMatch(target, filmFeel) {
  let d = 0;
  FEELS.forEach(f => { d += Math.abs((target[f.key] ?? 5) - filmFeel[f.key]); });
  return Math.max(0, Math.round(100 - (d / (FEELS.length * 10)) * 100));
}

// ── Why a film is recommended relative to a seed film ────────────────
function seedWhy(seed, film) {
...
// ── Sorted recommendation chain from a seed ──────────────────────────
function seedChain(seed) {
  if (!seed) return [];
  const ids = CHAINS[seed.id] || [];
  const inChain = ids.map(id => FILM_BY_ID[id]).filter(Boolean);
  const rest = FILMS.filter(f => f.id !== seed.id && !ids.includes(f.id))
    .sort((a, b) => feelMatch(seed.feel, b.feel) - feelMatch(seed.feel, a.feel));
  return [...inChain, ...rest];
}

Object.assign(window, {
  WN, useWN, Icon, Poster, ScorePill, ThemeChip, FacetChip, GlowButton,
  FeelSlider, Eyebrow, StatRing, feelMatch, seedWhy, seedChain,
});
