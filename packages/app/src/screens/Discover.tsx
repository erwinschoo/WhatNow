    ...
    <div style={{ height: "100%", scrollSnapAlign: "start", position: "relative", overflow: "hidden" }}>
      <div
        ...
        <div style={{ position: "absolute", right: 14, bottom: 188, display: "flex", flexDirection: "column", gap: 18, alignItems: "center", zIndex: 3 }}>
          ...
          <RailBtn icon="eye" label="Gezien" onClick={() => onSeen(film.id)} />
...
export function Discover() {
  const ctx = useWN();
  const { t, watchlist, toggleWatchlist, toggleSeen, openFilm, openTune, openSearch, seed, clearSeed, favoriteThemes, feelTarget, seen } = ctx;
  ...
  const films = seedFilm ? seedChain(seedFilm) : personalFeed({ favoriteThemes, feelTarget, seen });
    ...
    return (
      <div style={{ position: "absolute", inset: 0, paddingBottom: 88 }}>
        <div key={seed || "all"} className="wn-scroll" style={{ height: "100%", scrollSnapType: "y mandatory" }}>
          {films.map((f) => (
            <FeedSlide key={f.id} film={f} onOpen={openFilm} onWatchlist={toggleWatchlist} onSeen={toggleSeen} inList={watchlist.includes(f.id)} motion={motion} whyText={seedFilm ? seedWhy(seedFilm, f) : f.why} />
          ))}
        </div>
...
function DiscoverRows({ seedFilm, films }: { seedFilm: CatalogFilm | null; films: CatalogFilm[] }) {
  const { t, openFilm, openTune, openSearch, clearSeed } = useWN();
