/* Seed-zoeken: voer een film in, krijg een thematische keten — geport uit screens-search.jsx. */
...
export function Search() {
  const { closeSearch, setSeed, tr } = useWN();
  ...
  const suggestionIds = ["intowild", "br2049", "parasite", "itmfl", "drive", "arrival"];
  const suggestions = suggestionIds.map((id) => byId[id]).filter(Boolean) as CatalogFilm[];
  const surprise = () => setSeed(films[Math.floor(Math.random() * films.length)].id);

  const Row = ({ f }: { f: CatalogFilm }) => (
    <button onClick={() => setSeed(f.id)} style={{ display: "flex", gap: 13, alignItems: "center", width: "100%", textAlign: "left", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, padding: 10, cursor: "pointer" }}>
      <div style={{ width: 44, height: 66, flexShrink: 0, borderRadius: 9 }}><Poster film={f} showText={false} rounded={9} size="w185" /></div>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 5, marginTop: 7, flexWrap: "wrap" }}>
          {f.themes.slice(0, 2).map((t) => <span key={t} style={{ fontFamily: "var(--mono)", fontSize: 9, color: "var(--amber2)", background: "rgba(255,138,43,0.1)", padding: "2px 7px", borderRadius: 5 }}>{tr(t)}</span>)}
        </div>
      </div>
      <div style={{ fontFamily: "var(--mono)", fontSize: 9.5, color: "var(--amber2)", border: "1px solid rgba(255,138,43,0.4)", borderRadius: 999, padding: "5px 10px", flexShrink: 0 }}>{tr("Als seed")}</div>
    </button>
  );
    ...
    <div style={{ position: "absolute", inset: 0, background: "var(--bg)", zIndex: 45, display: "flex", flexDirection: "column", animation: "wnFade .2s" }}>
      <div style={{ padding: "52px 18px 14px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div>
            <Eyebrow>{tr("De thematische keten")}</Eyebrow>
            <div style={{ fontFamily: "var(--sans)", fontWeight: 700, fontSize: 21, color: "var(--tx)", lineHeight: 1, marginTop: 2 }}>{tr("Vind films zoals…")}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--surface2)", border: "1px solid var(--line2)", borderRadius: 14, padding: "0 14px", height: 52 }}>
          ...
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder={tr("Noem een film die je raakte…")}
        ...
        {query ? (
          results.length ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Eyebrow style={{ marginBottom: 2 }}>{tr("{n} resultaten", { n: results.length })}</Eyebrow>
              {results.map((f) => <Row key={f.id} f={f} />)}
            </div>
          ) : (
            <div style={{ textAlign: "center", padding: "50px 20px", color: "var(--tx3)" }}>
              <div style={{ fontFamily: "var(--sans)", fontSize: 15, color: "var(--tx2)" }}>{tr('Niets gevonden voor "{q}"', { q })}</div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, marginTop: 6 }}>{tr("Probeer een titel, regisseur of thema")}</div>
            </div>
          )
        ) : (
          <>
            <button onClick={surprise} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, background: "linear-gradient(120deg, rgba(255,122,24,0.14), rgba(255,179,71,0.07))", border: "1px solid rgba(255,138,43,0.3)", borderRadius: 14, padding: 15, cursor: "pointer", marginBottom: 22 }}>
              ...
              <div style={{ textAlign: "left" }}>
                <div style={{ fontFamily: "var(--sans)", fontWeight: 700, fontSize: 15, color: "var(--tx)" }}>{tr("Verras me")}</div>
                <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--tx3)", marginTop: 2 }}>{tr("Start vanaf een willekeurige film")}</div>
              </div>
            </button>
            <Eyebrow style={{ marginBottom: 12 }}>{tr("Probeer een seed")}</Eyebrow>
